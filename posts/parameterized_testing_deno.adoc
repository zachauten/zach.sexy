...
:image: denosaur-labcoat.webp
:imagesDir: /img/parameterized_testing_deno/
---

image:denosaur-labcoat.webp[a friendly brontosaurus wearing a labcoat and  holding a beaker]

I'd like to share a simple function I wrote for parameterized testing in https://deno.land[Deno].
...
api (Deno.test, to register tests), and a https://deno.land/std@0.107.0/testing[small handful of assertions] in the
...
test case on different inputs. For example, https://jestjs.io/blog/2018/05/29/jest-23-blazing-fast-delightful-testing#jest-each[Jest's "each"]
function.
